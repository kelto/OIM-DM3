...
Color compute_direct_lighting (Ray ray_, Isect isect_) {
    ...
    for(int i = 0; i<nb; i++)
    {
        ...
        {
            l2 = direct_lighting(ray_,isect_, light);
            l= add_color_color(l,l2);
...
Color trace_ray (Ray ray_) {
    ...
    Isect isect;
    Color l;
    if(!intersect_scene(&ray_, &isect))
        l = init_color(1.f,1.f,1.f);
    else {
        l = compute_direct_lighting(ray_, isect);
        if(ray_importance(ray_) > MIN_IMPORTANCE)
        {
            if(isect_has_reflection(isect) )
            {
                Ray ray_reflect;
                Color c_reflect = reflect(ray_, isect, &ray_reflect);
                Color c = trace_ray(ray_reflect);
                c = c*c_reflect;
            ...
            {
                Ray ray_refract;
                Color c_refract = refract(ray_, isect, &ray_refract);
                Color c = trace_ray(ray_refract);
                c = c*c_refract;
                l = l+c;
...
void compute_image () {
    ...
    for(int i = 0; i < x_res; i++)
    {
        ...
        {
            Ray ray = camera_ray(i,j);
            Color l = trace_ray(ray);
            set_pixel_color(i,j,l);
        }
    }
